...
import com.example.mailit.databinding.ActivityDraftBinding;
...
public class DraftActivity extends AppCompatActivity implements EmailAdapter.EmailAdapterCallback {
    ...
    private void setUpSwipeToDelete() {
        ItemTouchHelper.SimpleCallback simpleItemTouchCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                Email email = draftEmails.get(position);
                deleteFromServer(email);
                draftEmails.remove(position);
                adapter.notifyDataSetChanged();
                Toast.makeText(DraftActivity.this, R.string.draft_delete_success, Toast.LENGTH_SHORT).show();
            }
        };
    ...
    private void loadDraftEmails() {
        ...
        EmailAsyncTask getAllDraftTask = new EmailAsyncTask(getApplicationContext(), Action.GET_ALL_DRAFT, new ResultListener<Email>() {
            @Override
            public void onResult(Result<Email> result) {
                Log.d(TAG, "Result of loading draft emails from database: " + result);
                Error error = (result != null) ? result.getError() : null;
                List<Email> usersList = (result != null) ? result.getItems() : new ArrayList<Email>();
                if ((result == null) || (error != null)) {
                    String errorMsg = (error != null) ? error.getMessage() : getString(R.string.email_get_all_draft_error);
                    Toast.makeText(DraftActivity.this, errorMsg, Toast.LENGTH_LONG).show();
                    return;
                }
                draftEmails = usersList;
                adapter.emailsLoaded(draftEmails);
            }
        });
        getAllDraftTask.execute(email);
    }
    ...
    private void onSearchClicked(View view) {
        ...
        if (TextUtils.isEmpty(keyWord)) {
            ...
            Intent intent = new Intent(DraftActivity.this, SearchActivity.class);
            intent.putExtra(Constants.SEARCH_DRAFT, keyWord);
    ...
    public void onItemClicked(Email email, int position) {
        Intent intent = new Intent(DraftActivity.this, ComposeActivity.class);
        intent.putExtra(Constants.EMAIL, email);
        intent.putExtra(Constants.POSITION, position);
        startActivityForResult(intent, Constants.DRAFT_DELETE);
    }
}
